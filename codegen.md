...
| Виртуальный регистр | Физический регистр | Роль в программе |
| :--- | :--- | :--- |
| **v0** | **t0** | Аккумулятор результата (`res`) |
| **v1** | **t1** | Итератор цикла (`i`) |
| **v2** | **t2** | Граница цикла (`n - 1`) |
| **v3, v4, v7, v8** | **t2** | Временные адреса и значения (цикл) |
| **v5, v6** | **t3** | Временные значения (цикл) |
| **v9** | **t2** | Смещение в эпилоге |
| **v10, v11, v12** | **t1** | Адреса и значения в эпилоге (после смерти v1) |
